...
# =============================================================================
# ICON PATH DATA (in 100x100 viewBox, content from 5-95)
# =============================================================================

# These paths are for the standalone icon at 100x100 scale
ICON_PATHS_100 = {
    "left_bracket": "M 5.0 50.0L 22.0 8.0L 22.0 18.0L 12.0 50.0L 22.0 82.0L 22.0 92.0L 5.0 50.0Z",
    "right_bracket": "M 95.0 50.0L 78.0 8.0L 78.0 18.0L 88.0 50.0L 78.0 82.0L 78.0 92.0L 95.0 50.0Z",
    "icon_a": "M 46.5 57.8Q 46.5 59.2 47.0 59.6Q 47.4 60.2 48.2 60.4L 47.2 63.5Q 45.7 63.4 44.5 62.7Q 43.4 62.1 42.8 60.7Q 41.8 61.9 39.9 62.6Q 38.2 63.4 36.2 63.4Q 33.1 63.4 31.1 61.6Q 29.2 59.9 29.2 57.1Q 29.2 53.6 32.1 51.8Q 34.6 50.0 39.5 50.0H 42.4V 48.8Q 42.4 46.7 41.2 45.7Q 40.0 44.9 37.9 44.9Q 36.8 44.9 35.4 45.1Q 33.7 45.5 32.3 45.9L 31.3 42.8Q 33.1 42.2 34.9 41.8Q 36.8 41.4 38.4 41.4Q 42.5 41.4 44.5 43.2Q 46.5 45.0 46.5 48.3ZM 37.5 60.2Q 38.9 60.2 40.2 59.5Q 41.4 58.6 42.4 57.4V 52.6H 40.0Q 36.9 52.6 35.6 53.8Q 34.1 54.9 34.1 56.8Q 34.1 60.2 37.5 60.2Z",
    "icon_g": "M 71.7 39.4L 73.0 43.0Q 71.7 43.5 70.2 43.7Q 68.8 43.8 66.9 43.8Q 68.8 44.8 70.0 46.0Q 70.8 47.2 70.8 49.0Q 70.8 51.1 69.7 52.8Q 68.6 54.5 66.7 55.4Q 65.0 56.2 62.3 56.2Q 61.5 56.2 60.6 56.2Q 60.0 56.1 59.4 55.9Q 58.6 56.6 58.6 57.6Q 58.6 58.2 59.0 58.6Q 59.6 59.2 61.0 59.2H 64.8Q 67.1 59.2 68.8 60.0Q 70.5 60.6 71.5 62.0Q 72.5 63.2 72.5 64.8Q 72.5 67.9 69.9 69.7Q 67.3 71.4 62.3 71.4Q 58.8 71.4 56.7 70.6Q 54.6 69.8 53.8 68.4Q 53.0 66.8 53.0 64.6H 57.0Q 57.0 65.8 57.5 66.5Q 58.2 67.4 59.4 67.6Q 60.7 68.0 62.9 68.0Q 66.2 68.0 67.6 67.0Q 68.9 66.0 68.9 64.5Q 68.9 63.2 67.8 62.5Q 66.4 61.7 64.5 61.7H 60.6Q 57.6 61.7 56.2 60.6Q 54.8 59.2 54.8 57.5Q 54.8 55.2 57.0 53.8Q 55.3 52.7 54.4 51.3Q 53.5 49.8 53.5 47.8Q 53.5 45.6 54.7 43.9Q 55.8 42.0 57.9 41.1Q 59.8 40.2 62.3 40.2Q 64.8 40.4 66.6 40.0Q 68.4 39.6 69.8 38.8Q 71.0 38.4 71.7 39.4ZM 62.3 44.4Q 60.2 44.4 59.0 45.6Q 57.8 46.8 57.8 48.8Q 57.8 50.8 59.0 52.0Q 60.2 53.0 62.3 53.0Q 64.4 53.0 65.5 52.0Q 66.5 50.8 66.5 48.8Q 66.5 44.4 62.3 44.4Z",
}

# Content bounds for icon (100x100 viewBox)
ICON_CONTENT_BOUNDS = {
    "left": 5.0,
    "right": 95.0,
    "top": 8.0,
...
# =============================================================================
# WORDMARK PATH DATA (in regular coordinate space, viewBox 0 0 203 64)
# =============================================================================
...
# Text paths for "agentprompt" (in regular coordinate space)
TEXT_PATHS = {
...
# Wordmark content bounds
WORDMARK_CONTENT_BOUNDS = {
    "left_x": 10.4,
    "right_x": 193.4,
    "top_y": 14,
    "bottom_y": 50,
...
def scale_and_translate_path(
    ...
    """Scale path around source center, then translate to destination center."""
    result = []
    tokens = re.findall(r"[MmLlHhVvCcSsQqTtAaZz]|[-+]?[0-9]*\.?[0-9]+", path_str)
        ...
        if token.isalpha() and len(token) == 1:
            ...
            if cmd == "H":
                ...
                if pos == 5:
                    ...
                    is_y = True
                elif pos in (0, 1):  # rx, ry - scale these
                    val = val * scale
            elif cmd in ("M", "L", "T"):
                is_x = (coord_idx % 2) == 0
                is_y = (coord_idx % 2) == 1
            ...
            if is_x:
                val = dst_center_x + (val - src_center_x) * scale
            elif is_y:
                val = dst_center_y + (val - src_center_y) * scale
...
def generate_icon(
    variant: str,  # "rounded", "square", "transparent", "adaptive", "white-bg"
    tight: bool = False,
) -> str:
    """Generate an icon SVG."""
    colors = CONFIG["colors"]
    icon_cfg = CONFIG["icon"]["tight" if tight else "regular"]

    viewbox_size = CONFIG["icon"]["regular"]["viewbox_size"]
    padding = icon_cfg["padding"]
    corner_radius = icon_cfg["corner_radius"]

    # Calculate scale and translation for tight version
    src_center_x = (ICON_CONTENT_BOUNDS["left"] + ICON_CONTENT_BOUNDS["right"]) / 2
    src_center_y = (ICON_CONTENT_BOUNDS["top"] + ICON_CONTENT_BOUNDS["bottom"]) / 2

    # For tight, we scale content to fit new padding
    regular_padding = CONFIG["icon"]["regular"]["padding"]
    if tight:
        # Scale factor to fit content with new padding
        available_size = viewbox_size - 2 * padding
        regular_available = viewbox_size - 2 * regular_padding
        scale = available_size / regular_available
        dst_center_x = viewbox_size / 2
        dst_center_y = viewbox_size / 2

        def transform(path: str) -> str:
            return scale_and_translate_path(
                path, scale, src_center_x, src_center_y, dst_center_x, dst_center_y
            )
    else:
    ...
    if variant == "transparent":
        ...
        return generate_adaptive_icon(viewbox_size, corner_radius, transform)
    elif variant == "white-bg":
        has_background = True
    ...
    paths.append(
        make_path_element(
            transform(ICON_PATHS_100["left_bracket"]), colors["amber"], "Left bracket"
        )
    )
    paths.append(
        make_path_element(transform(ICON_PATHS_100["icon_a"]), icon_ag_color, "'a'")
    )
    paths.append(
        make_path_element(transform(ICON_PATHS_100["icon_g"]), icon_ag_color, "'g'")
    )
    paths.append(
        make_path_element(
            transform(ICON_PATHS_100["right_bracket"]), colors["amber"], "Right bracket"
        )
    )

    if has_background:
        bg_elem = f'  <rect width="{viewbox_size}" height="{viewbox_size}" rx="{corner_radius}" fill="{bg_color}"/>'
    else:
        bg_elem = ""
...
def generate_adaptive_icon(viewbox_size: int, corner_radius: int, transform) -> str:
    """Generate adaptive icon that responds to system color scheme."""
    colors = CONFIG["colors"]

    bracket_left = make_path_element(
        transform(ICON_PATHS_100["left_bracket"]), colors["amber"]
    )
    bracket_right = make_path_element(
        transform(ICON_PATHS_100["right_bracket"]), colors["amber"]
    )

    ag_light = [
        make_path_element(transform(ICON_PATHS_100["icon_a"]), colors["cream"]),
        make_path_element(transform(ICON_PATHS_100["icon_g"]), colors["cream"]),
    ]

    ag_dark = [
        make_path_element(transform(ICON_PATHS_100["icon_a"]), colors["dark_text"]),
        make_path_element(transform(ICON_PATHS_100["icon_g"]), colors["dark_text"]),
    ]
  ...
  <rect class="bg-dark" width="{viewbox_size}" height="{viewbox_size}" rx="{corner_radius}" fill="{colors["dark_bg"]}"/>
  <rect class="bg-light" width="{viewbox_size}" height="{viewbox_size}" rx="{corner_radius}" fill="{colors["light_bg"]}"/>
  
{bracket_left}
{bracket_right}
  
  <g class="text-light">
{chr(10).join(ag_light)}
  </g>
...
def generate_favicon() -> str:
    """Generate adaptive favicon at 32x32."""
    colors = CONFIG["colors"]
    cfg = CONFIG["favicon"]
    viewbox_size = cfg["viewbox_size"]
    corner_radius = cfg["corner_radius"]
    padding = cfg["padding"]

    # Scale from 100x100 icon to 32x32 with padding
    src_center = 50.0  # Center of 100x100
    dst_center = viewbox_size / 2
    available = viewbox_size - 2 * padding
    scale = available / 90.0  # Original content spans 90 units (5-95)

    def transform(path: str) -> str:
        return scale_and_translate_path(
            path, scale, src_center, src_center, dst_center, dst_center
        )
...
def generate_wordmark(
    ...
    if tight:
        content_left = WORDMARK_CONTENT_BOUNDS["left_x"]
        content_right = WORDMARK_CONTENT_BOUNDS["right_x"]
        content_width = content_right - content_left
        content_top = WORDMARK_CONTENT_BOUNDS["top_y"]
        content_bottom = WORDMARK_CONTENT_BOUNDS["bottom_y"]
        content_height = content_bottom - content_top

        h_padding = CONFIG["wordmark"]["tight"]["horizontal_padding"]
        v_padding = CONFIG["wordmark"]["tight"]["vertical_padding"]
        corner_radius = CONFIG["wordmark"]["tight"]["corner_radius"]
        ...
        corner_radius = CONFIG["wordmark"]["regular"]["corner_radius"]
        viewbox_width = CONFIG["wordmark"]["regular"]["viewbox_width"]
        viewbox_height = CONFIG["wordmark"]["regular"]["viewbox_height"]
    ...
    paths.append(
        make_path_element(
            transform(WORDMARK_ICON_PATHS["left_bracket"]),
            colors["amber"],
            "Left bracket",
        )
    )
    paths.append(
        make_path_element(
            transform(WORDMARK_ICON_PATHS["icon_a"]), icon_ag_color, "Icon 'a'"
        )
    )
    paths.append(
        make_path_element(
            transform(WORDMARK_ICON_PATHS["icon_g"]), icon_ag_color, "Icon 'g'"
        )
    )
    paths.append(
        make_path_element(
            transform(WORDMARK_ICON_PATHS["right_bracket"]),
            colors["amber"],
            "Right bracket",
        )
    )
...
def generate_adaptive_wordmark(
    ...
    bracket_left = make_path_element(
        transform(WORDMARK_ICON_PATHS["left_bracket"]), colors["amber"]
    )
    bracket_right = make_path_element(
        transform(WORDMARK_ICON_PATHS["right_bracket"]), colors["amber"]
    )

    icon_ag_light = [
        make_path_element(transform(WORDMARK_ICON_PATHS["icon_a"]), colors["cream"]),
        make_path_element(transform(WORDMARK_ICON_PATHS["icon_g"]), colors["cream"]),
    ]

    icon_ag_dark = [
        make_path_element(
            transform(WORDMARK_ICON_PATHS["icon_a"]), colors["dark_text"]
        ),
        make_path_element(
            transform(WORDMARK_ICON_PATHS["icon_g"]), colors["dark_text"]
        ),
    ]
...
def main():
    ...
    wordmark_dir = base_dir / subdirs["wordmark"]
    icon_dir = base_dir / subdirs["icon"]
    favicon_dir = base_dir / subdirs["favicon"]

    for d in [wordmark_dir, icon_dir, favicon_dir]:
        d.mkdir(parents=True, exist_ok=True)

    print("=" * 60)
    print("Agent Prompt Logo Generator")
    print("=" * 60)
    print()

    # Generate wordmarks
    print(f"WORDMARKS → {subdirs['wordmark']}/")
    print("-" * 40)
    wm_reg = CONFIG["wordmark"]["regular"]
    wm_tight = CONFIG["wordmark"]["tight"]
    print(
        f"  Regular: {wm_reg['viewbox_width']}×{wm_reg['viewbox_height']}, padding {wm_reg['horizontal_padding']}h/{wm_reg['vertical_padding']}v"
    )
    print(
        f"  Tight:   padding {wm_tight['horizontal_padding']}h/{wm_tight['vertical_padding']}v"
    )
    print()

    wordmark_variants = ["dark", "light", "white", "adaptive"]
    for variant in wordmark_variants:
        svg = generate_wordmark(variant, tight=False)
        filename = f"wordmark-{variant}.svg"
        (wordmark_dir / filename).write_text(svg)
        print(f"  ✓ {filename}")
    ...
    # Generate icons
    print(f"ICONS → {subdirs['icon']}/")
    print("-" * 40)
    ic_reg = CONFIG["icon"]["regular"]
    ic_tight = CONFIG["icon"]["tight"]
    print(
        f"  Regular: {ic_reg['viewbox_size']}×{ic_reg['viewbox_size']}, padding {ic_reg['padding']}, rx {ic_reg['corner_radius']}"
    )
    print(f"  Tight:   padding {ic_tight['padding']}, rx {ic_tight['corner_radius']}")
    print()

    icon_variants = ["rounded", "square", "transparent", "adaptive", "white-bg"]
    for variant in icon_variants:
        svg = generate_icon(variant, tight=False)
        filename = f"icon-{variant}.svg"
        (icon_dir / filename).write_text(svg)
        print(f"  ✓ {filename}")

        svg = generate_icon(variant, tight=True)
        filename = f"icon-{variant}-tight.svg"
        (icon_dir / filename).write_text(svg)
        print(f"  ✓ {filename}")
    ...
    # Generate favicon
    print(f"FAVICON → {subdirs['favicon']}/")
    print("-" * 40)
    fav = CONFIG["favicon"]
    print(
        f"  Size: {fav['viewbox_size']}×{fav['viewbox_size']}, padding {fav['padding']}, rx {fav['corner_radius']}"
    )
    print()

    svg = generate_favicon()
    filename = "favicon-adaptive.svg"
    (favicon_dir / filename).write_text(svg)
    print(f"  ✓ {filename}")

    print()
    print("=" * 60)
    print(f"Done! Generated files in: {base_dir.absolute()}")
    print()
    print("To adjust settings, edit CONFIG at the top of the script.")
    print("=" * 60)
